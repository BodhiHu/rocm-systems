...
fn main() {
    ...
    println!(
        "cargo:rerun-if-changed={}",
        repo_root
            .join("projects/amdsmi/include/libdrm/amdgpu_drm.h")
            .display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        repo_root
            .join("projects/rocr-runtime/libhsakmt/include/hsakmt/drm/amdgpu_drm.h")
            .display()
    );
...
fn find_repo_root(manifest_dir: &Path) -> PathBuf {
    manifest_dir
        .ancestors()
        .find(|path| path.join("projects").is_dir() && path.join("experimental").is_dir())
        .unwrap_or_else(|| {
            panic!(
