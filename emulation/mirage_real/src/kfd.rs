...
use mirage_schema::amdgpu::{
    ...
    IoctlCtx, KfdCriuBoBucket, KfdCriuDeviceBucket, KfdDbgDeviceInfoEntry, KfdDbgTrapArgs,
    KfdDbgTrapOp, KfdEventData, KfdHwExceptionData, KfdMemoryExceptionData,
    KfdMemoryExceptionFailure, KfdMemoryRange, KfdPcSampleArgs, KfdPcSampleInfo, KfdPmcSettings,
    KfdProcessDeviceAperture, KfdProfilerArgs, KfdQueueSnapshotEntry, KfdSignalEventData,
    KfdSvmAttribute,
};
...
impl HandleKfdIoctl for RealEmulator {
    ...
    fn amdkfd_ioc_criu_op(
        ...
        let mut devices: Vec<KfdCriuDeviceBucketRaw> =
            request.devices.iter().copied().map(Into::into).collect();
        let mut bos: Vec<KfdCriuBoBucketRaw> =
            request.bos.iter().copied().map(Into::into).collect();
        let mut priv_data = request.priv_data;
        let mut args = kfd::kfd_ioctl_criu_args {
            devices: maybe_mut_ptr(&mut devices),
            bos: maybe_mut_ptr(&mut bos),
...
fn dbg_trap_to_raw(
    ...
    raw.__bindgen_anon_1 = match request.op {
        KfdDbgTrapOp::Enable => {
            kfd::kfd_ioctl_dbg_trap_args__bindgen_ty_1 {
                enable: {
                    let enable = request.args.enable.unwrap_or(
                        mirage_schema::amdgpu::KfdDbgTrapEnableArgs {
                            exception_mask: 0,
                            rinfo_size: 0,
                            dbg_fd: 0,
                        },
        ...
        KfdDbgTrapOp::SetExceptionsEnabled => kfd::kfd_ioctl_dbg_trap_args__bindgen_ty_1 {
            set_exceptions_enabled: {
                let args = request.args.set_exceptions_enabled.unwrap_or(
                    mirage_schema::amdgpu::KfdDbgTrapSetExceptionsEnabledArgs { exception_mask: 0 },
                );
                kfd::kfd_ioctl_dbg_trap_set_exceptions_enabled_args {
                    exception_mask: args.exception_mask,
                }
        ...
        KfdDbgTrapOp::ResumeQueues => {
            let args = request.args.resume_queues.clone().unwrap_or(
                mirage_schema::amdgpu::KfdDbgTrapResumeQueuesArgs {
                    queue_ids: Vec::new(),
                },
            );
            owned.queue_ids = args.queue_ids;
        ...
        KfdDbgTrapOp::SetFlags => kfd::kfd_ioctl_dbg_trap_args__bindgen_ty_1 {
            set_flags: {
                let args = request
                    .args
                    .set_flags
                    .unwrap_or(mirage_schema::amdgpu::KfdDbgTrapSetFlagsArgs { flags: 0 });
                kfd::kfd_ioctl_dbg_trap_set_flags_args {
                    flags: args.flags,
                    pad: 0,
                }
...
fn dbg_trap_from_raw(
    ...
    unsafe {
        match op {
            ...
            KfdDbgTrapOp::SendRuntimeEvent => {
                let event = raw.__bindgen_anon_1.send_runtime_event;
                args.send_runtime_event =
                    Some(mirage_schema::amdgpu::KfdDbgTrapSendRuntimeEventArgs {
                        exception_mask: event.exception_mask,
                        gpu_id: event.gpu_id,
                        queue_id: event.queue_id,
                    });
            ...
            KfdDbgTrapOp::SetExceptionsEnabled => {
                args.set_exceptions_enabled =
                    Some(mirage_schema::amdgpu::KfdDbgTrapSetExceptionsEnabledArgs {
                        exception_mask: raw.__bindgen_anon_1.set_exceptions_enabled.exception_mask,
                    });
            }
            KfdDbgTrapOp::SetWaveLaunchOverride => {
                let value = raw.__bindgen_anon_1.launch_override;
                args.launch_override =
                    Some(mirage_schema::amdgpu::KfdDbgTrapSetWaveLaunchOverrideArgs {
                        override_mode: std::mem::transmute(value.override_mode),
                        enable_mask: value.enable_mask,
                        support_request_mask: value.support_request_mask,
                    });
            ...
            KfdDbgTrapOp::SetNodeAddressWatch => {
                ...
                args.set_node_address_watch =
                    Some(mirage_schema::amdgpu::KfdDbgTrapSetNodeAddressWatchArgs {
                        address: value.address,
                        mode: std::mem::transmute(value.mode),
                        mask: value.mask,
                        gpu_id: value.gpu_id,
            ...
            KfdDbgTrapOp::ClearNodeAddressWatch => {
                ...
                args.clear_node_address_watch =
                    Some(mirage_schema::amdgpu::KfdDbgTrapClearNodeAddressWatchArgs {
                        gpu_id: value.gpu_id,
                        id: value.id,
                    });
            }
            ...
            KfdDbgTrapOp::QueryDebugEvent => {
                ...
                args.query_debug_event =
                    Some(mirage_schema::amdgpu::KfdDbgTrapQueryDebugEventArgs {
                        exception_mask: value.exception_mask,
                        gpu_id: value.gpu_id,
                        queue_id: value.queue_id,
                    });
            ...
            KfdDbgTrapOp::QueryExceptionInfo => {
                let value = raw.__bindgen_anon_1.query_exception_info;
                args.query_exception_info =
                    Some(mirage_schema::amdgpu::KfdDbgTrapQueryExceptionInfoArgs {
                        info: owned.info.clone(),
                        info_size: value.info_size,
                        source_id: value.source_id,
                        exception_code: value.exception_code,
