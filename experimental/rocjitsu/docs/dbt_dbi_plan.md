...
- **Classification rules** (which `Action` each instruction gets) — auto-generated by `amdisa/legalization.py` from the ISA XML specs, stored as `inline constexpr InstructionLegalization[]` arrays in per-pair headers under `isa/dbt/generated/legalization_*.h`. Each entry maps a `(src_encoding_id, src_opcode)` pair to an `Action` value and a `target_opcode` (populated for Identity, Substitute, and Lower entries; used by `BinaryTranslator`). Adding a new rule means updating the mnemonic rename map or domain-specific rules in `legalization.py` and regenerating.
...
// isa/dbt/generated/legalization_tables.h.  The types Action, InstructionLegalization,
// and lookup() are defined ONCE in the generated header — InstructionMapper
// does NOT duplicate them.

#include "rocjitsu/isa/dbt/generated/legalization_tables.h"
...
/// InstructionLegalization struct are defined in isa/dbt/generated/legalization_tables.h
...
```cpp
// isa/dbt/generated/legalization_tables.h — AUTO-GENERATED by amdisa
...
One header is generated per (src, dst) pair (e.g., `isa/dbt/generated/encoding_cdna4_to_rdna4.h`). Adding a new pair reuses existing decode/encode functions and only generates the missing ones.

`BinaryTranslator::lower_encoding()` dispatches to the generated pair's translate function:

```cpp
TranslationResult BinaryTranslator::lower_encoding(const Instruction &inst) {
    return translate_encoding_cdna4_to_rdna4(
        inst.encoding_id(), inst.raw_word(0), inst.raw_word(1), inst.raw_word(2),
        dst_opcode, inst.seg());
}
```
...
| Coherency remap helpers and `TranslationResult` struct | `lib/rocjitsu/src/rocjitsu/isa/dbt/encoding_translator.h` (hand-written, shared across all pairs) |
...
One header is generated per (src, dst) pair: `isa/dbt/generated/encoding_<src>_to_<dst>.h`. The shared header `isa/dbt/encoding_translator.h` provides coherency remap helpers and `TranslationResult`. Adding a new pair requires only running the codegen with different XML specs. For the MVP, only `cdna4_to_rdna4` is generated.
...
python -m amdisa --multi cdna4:cdna4.xml rdna4:rdna4.xml --gen-legalization \
    --legalization-output isa/dbt/generated
```
...
- `lib/rocjitsu/src/rocjitsu/isa/dbt/generated/legalization_tables.h` — auto-generated legalization tables
- `lib/python/amdisa/legalization.py` — legalization table generator
- `lib/python/amdisa/legalization_codegen.py` — C++20 legalization table codegen
- `lib/python/amdisa/encoding_translator_codegen.py` — C++20 encoding translator codegen
- `lib/rocjitsu/src/rocjitsu/isa/dbt/encoding_translator.h` — coherency remap helpers and `TranslationResult` (shared across all pairs)
- `lib/rocjitsu/src/rocjitsu/isa/dbt/generated/encoding_fields.h` — ISA-neutral field structs (union across all 9 ISAs, shared)
- `lib/rocjitsu/src/rocjitsu/isa/dbt/generated/encoding_<src>_to_<dst>.h` — auto-generated decode/encode functions and dispatch per pair
