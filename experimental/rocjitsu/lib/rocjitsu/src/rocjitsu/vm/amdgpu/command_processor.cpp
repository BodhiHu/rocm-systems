...
void CommandProcessor::process_aql_packet(const hsa_kernel_dispatch_packet_t &pkt,
  ...
  // Map host memory for KFD dispatches.
  if (host_accessible && memory_) {
    uint64_t code_base = pkt.kernel_object & ~0xFFFULL;
    constexpr size_t CODE_MAP_SIZE = 2 << 20;
    ...
    if (karg != 0) {
      ...
      constexpr size_t KARG_MAP_SIZE = 8 * 4096;
  ...
  dp.scratch_backing_addr = dp.scratch_backing_addr; // already set above
...
// See amdgpu driver's SDMA headers.
namespace sdma {
...
constexpr uint8_t OP_HDP_FLUSH = 0x26;
