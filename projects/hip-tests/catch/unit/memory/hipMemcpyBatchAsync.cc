...
std::vector<std::pair<int, int>> GetPeerAccessibleDevicePairs() {
  ...
  for (int src_device = 0; src_device < device_count; ++src_device) {
    for (int dst_device = 0; dst_device < device_count; ++dst_device) {
      ...
      HIP_CHECK(
          hipDeviceCanAccessPeer(&can_access_peer, src_device, dst_device));
...
std::vector<LinearAllocGuard<int>>
AllocateBatchBuffers(LinearAllocs allocation_type, const BatchConfig &config,
...
std::vector<void *>
MakeBatchPtrs(std::vector<LinearAllocGuard<int>> &allocations,
              size_t offset_bytes = 0) {
  std::vector<void *> ptrs;

  for (LinearAllocGuard<int> &allocation : allocations) {
    ptrs.push_back(static_cast<void *>(
        reinterpret_cast<char *>(allocation.ptr()) + offset_bytes));
  }

  return ptrs;
}

void FillDeviceBuffers(const std::vector<void *> &ptrs, size_t copy_size,
                       int value) {
  ...
  for (size_t i = 0; i < ptrs.size(); ++i) {
    ...
    HIP_CHECK(
        hipMemcpy(ptrs[i], source.data(), copy_size, hipMemcpyHostToDevice));
  }
}
...
void VerifyArrayFromBothEnds(const int *values, size_t copy_elements,
                             int expected, size_t copy_index) {
  for (size_t offset = 0; offset < (copy_elements + 1) / 2; ++offset) {
    ...
    INFO("Array failure at copy index " << copy_index << ", element "
                                        << front_index);
...
void VerifyDeviceBuffers(const std::vector<void *> &ptrs, size_t copy_size,
  ...
  for (size_t i = 0; i < ptrs.size(); ++i) {
    HIP_CHECK(
        hipMemcpy(result.data(), ptrs[i], copy_size, hipMemcpyDeviceToHost));
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_Negative) {
  ...
  for (size_t i = 0; i < kCount; ++i) {
    src_allocs[i] =
        LinearAllocGuard<int>(LinearAllocs::hipMalloc, kSmallCopySize);
    dst_allocs[i] =
        LinearAllocGuard<int>(LinearAllocs::hipMalloc, kSmallCopySize);
  ...
  SECTION("Null destination array") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(nullptr, src_ptrs.data(), sizes.data(),
                                        kCount, nullptr, attrs_idxs, 0, nullptr,
                                        nullptr),
                    hipErrorInvalidValue);
  }

  SECTION("Null source array") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), nullptr, sizes.data(),
                                        kCount, nullptr, attrs_idxs, 0, nullptr,
                                        nullptr),
                    hipErrorInvalidValue);
  }
  ...
  SECTION("Null sizes array") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        nullptr, kCount, nullptr, attrs_idxs, 0,
                                        nullptr, nullptr),
                    hipErrorInvalidValue);
  }
  ...
  SECTION("Zero count") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        sizes.data(), 0, nullptr, attrs_idxs, 0,
                                        nullptr, nullptr),
                    hipErrorInvalidValue);
  }
  ...
  SECTION("Null destination element") {
    dst_ptrs[1] = nullptr;
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        sizes.data(), kCount, nullptr,
                                        attrs_idxs, 0, nullptr, nullptr),
                    hipErrorInvalidValue);
  }
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_Attrs_Negative) {
  ...
  SECTION("Null attrs with nonzero numAttrs") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        sizes.data(), kCount, nullptr,
                                        attrs_idxs.data(), 1, nullptr, nullptr),
                    hipErrorInvalidValue);
  }

  SECTION("Null attrsIdxs with nonzero numAttrs") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        sizes.data(), kCount, attrs.data(),
  ...
  SECTION("numAttrs exceeds count") {
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        sizes.data(), kCount, attrs.data(),
                                        attrs_idxs.data(), kCount + 1, nullptr,
                                        nullptr),
                    hipErrorInvalidValue);
  }
  ...
  SECTION("attrsIdxs is not monotonic") {
    ...
    HIP_CHECK_ERROR(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                        sizes.data(), kCount, attrs.data(),
                                        attrs_idxs.data(), 2, nullptr, nullptr),
                    hipErrorInvalidValue);
  }
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_Attrs_Functional) {
  ...
  SECTION("Regular attributes") {
    std::array<hipMemcpyAttributes, 6> host_attrs{
        hipMemcpyAttributes{
            hipMemcpySrcAccessOrderStream, {}, {}, hipMemcpyFlagDefault},
        hipMemcpyAttributes{
            hipMemcpySrcAccessOrderStream, {}, {}, hipMemcpyFlagExtPreferCE},
        hipMemcpyAttributes{
            hipMemcpySrcAccessOrderDuringApiCall, {}, {}, hipMemcpyFlagDefault},
        hipMemcpyAttributes{hipMemcpySrcAccessOrderDuringApiCall,
                            {},
                            {},
                            hipMemcpyFlagExtPreferCE},
        hipMemcpyAttributes{
            hipMemcpySrcAccessOrderAny, {}, {}, hipMemcpyFlagDefault},
        hipMemcpyAttributes{
            hipMemcpySrcAccessOrderAny, {}, {}, hipMemcpyFlagExtPreferCE},
    };
    ...
    HIP_CHECK(hipMemcpyBatchAsync(dst_ptrs.data(), host_src_ptrs.data(),
                                  sizes.data(), sizes.size(), host_attrs.data(),
                                  attrs_idxs.data(), attrs_idxs.size(), nullptr,
                                  nullptr));
  ...
  SECTION("Swap attribute") {
    ...
    hipError_t status = hipMemcpyBatchAsync(
        d2d_dst_ptrs.data(), d2d_src_ptrs.data(), sizes.data(),
        d2d_src_ptrs.size(), &attr, attrs_idxs, 1, nullptr, nullptr);
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_D2D_Functional) {
  ...
  const size_t copy_size =
      GENERATE(kSmallCopySize, kMediumCopySize, kLargeCopySize);
  const PointerPattern pointer_pattern = GENERATE(
      PointerPattern::BasePointers, PointerPattern::OffsetPointers,
      PointerPattern::UnalignedPointers, PointerPattern::BroadcastSource);
  const hipMemcpyFlags flag =
      GENERATE(hipMemcpyFlagDefault, hipMemcpyFlagExtPreferCE);
  const size_t offset_bytes =
      pointer_pattern == PointerPattern::OffsetPointers      ? sizeof(int)
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_H2D_Functional) {
  const LinearAllocs host_alloc_type =
  ...
  BatchConfig config{copy_count, copy_size};
  std::vector<LinearAllocGuard<int>> src =
      AllocateBatchBuffers(host_alloc_type, config);
  std::vector<LinearAllocGuard<int>> dst =
      AllocateBatchBuffers(LinearAllocs::hipMalloc, config);
  std::vector<void *> src_ptrs = MakeBatchPtrs(src);
  std::vector<void *> dst_ptrs = MakeBatchPtrs(dst);
  ...
  HIP_CHECK(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(), sizes.data(),
                                copy_count, nullptr, nullptr, 0, nullptr,
                                nullptr));
  HIP_CHECK(hipStreamSynchronize(nullptr));
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_Mixed_Functional) {
  ...
  std::array<void *, 4> src_ptrs{h2d_src.ptr(), d2d_src.ptr(), d2h_src.ptr(),
                                 h2h_src.ptr()};
  std::array<void *, 4> dst_ptrs{h2d_dst.ptr(), d2d_dst.ptr(), d2h_dst.ptr(),
                                 h2h_dst.ptr()};
  ...
  HIP_CHECK(hipMemcpy(d2d_src.ptr(), source.data(), copy_size,
                      hipMemcpyHostToDevice));
  std::fill(source.begin(), source.end(), kPatternValue + 2);
  HIP_CHECK(hipMemcpy(d2h_src.ptr(), source.data(), copy_size,
                      hipMemcpyHostToDevice));

  HIP_CHECK(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(), sizes.data(),
                                sizes.size(), nullptr, nullptr, 0, nullptr,
                                nullptr));
  HIP_CHECK(hipStreamSynchronize(nullptr));

  std::vector<int> result(copy_elements);
  HIP_CHECK(hipMemcpy(result.data(), h2d_dst.ptr(), copy_size,
                      hipMemcpyDeviceToHost));
  VerifyArrayFromBothEnds(result.data(), copy_elements, kPatternValue, 0);
  HIP_CHECK(hipMemcpy(result.data(), d2d_dst.ptr(), copy_size,
                      hipMemcpyDeviceToHost));
  VerifyArrayFromBothEnds(result.data(), copy_elements, kPatternValue + 1, 1);
  VerifyArrayFromBothEnds(d2h_dst.host_ptr(), copy_elements, kPatternValue + 2,
                          2);
  VerifyArrayFromBothEnds(h2h_dst.host_ptr(), copy_elements, kPatternValue + 3,
                          3);
}
...
HIP_TEST_CASE(Unit_hipMemcpyBatchAsync_Stream) {
  ...
  SECTION("Default stream") {
    ...
    HIP_CHECK(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                  sizes.data(), kCount, nullptr, nullptr, 0,
                                  nullptr, nullptr));
    HIP_CHECK(hipStreamSynchronize(nullptr));
  ...
  SECTION("Ordering after prior kernel on same stream") {
    ...
    for (size_t i = 0; i < kCount; ++i) {
      VectorSet<<<1, 256, 0, stream_guard.stream()>>>(
          static_cast<int *>(src_ptrs[i]), kPatternValue + static_cast<int>(i),
          kCopyElements);
    }
    HIP_CHECK(hipGetLastError());
    HIP_CHECK(hipMemcpyBatchAsync(dst_ptrs.data(), src_ptrs.data(),
                                  sizes.data(), kCount, nullptr, nullptr, 0,
                                  nullptr, stream_guard.stream()));
