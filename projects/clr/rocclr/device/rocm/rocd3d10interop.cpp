...
void Acquire(const Device* device, ID3D10Resource* d3d10Resource) {
  std::lock_guard<std::mutex> sl(gExtCacheLock);
  auto it = gExtCache.find(device);
  if (it != gExtCache.end() && it->second.pCLExt) {
    UINT gpuMask = 1 << 0;  // Only 1 GPU in Rocr backend
    it->second.pCLExt->CLAcquireResource(d3d10Resource, &gpuMask);
  }
}

void Release(const Device* device, ID3D10Resource* d3d10Resource) {
  std::lock_guard<std::mutex> sl(gExtCacheLock);
  auto it = gExtCache.find(device);
  if (it != gExtCache.end() && it->second.pCLExt) {
    UINT gpuMask = 1 << 0;  // Only 1 GPU in Rocr backend
    it->second.pCLExt->CLReleaseResource(d3d10Resource, &gpuMask);
  }
}
