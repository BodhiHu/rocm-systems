...
bool Export(const Memory* memory, ID3D11Resource* d3d11Resource,
  ...
  // Query DXGI shared handle for memory mapping
...
void Acquire(const Device* device, ID3D11Resource* d3d11Resource) {
  std::lock_guard<std::mutex> sl(gExtCacheLock);
  auto it = gExtCache.find(device);
  if (it != gExtCache.end() && it->second.pCLExt) {
    UINT gpuMask = 1 << 0;  // Only 1 GPU in Rocr backend
    it->second.pCLExt->CLAcquireResource11(d3d11Resource, &gpuMask);
  }
}

void Release(const Device* device, ID3D11Resource* d3d11Resource) {
  std::lock_guard<std::mutex> sl(gExtCacheLock);
  auto it = gExtCache.find(device);
  if (it != gExtCache.end() && it->second.pCLExt) {
    UINT gpuMask = 1 << 0;  // Only 1 GPU in Rocr backend
    it->second.pCLExt->CLReleaseResource11(d3d11Resource, &gpuMask);
  }
}
