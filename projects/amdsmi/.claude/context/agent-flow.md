...
Top-down view of how a user request travels through the agent system: the Router
triages intent, then either answers inline or routes to one of the three agents.
Solid arrows are sequential skill flow within an agent; dashed arrows are
agent-to-agent dispatch (Planning calls down into the shared Development and
Review agents).

```mermaid
flowchart TD
    User([User request])
    User --> Router{{AMD-SMI Router · thin triage}}

    Router --> Plan[Planning Agent]
    Router --> Dev[Development Agent]
    Router --> Rev[Review Agent]
    Router --> Inline[Inline answer<br/>trivial only]
