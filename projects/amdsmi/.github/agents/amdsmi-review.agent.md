...
You may be invoked directly by the user or handed off by the `AMD-SMI` router or
Planning agent. When handed off, read the `handoff` doc (see the `handoff` skill)
for the branch/PR, scope, and any focus modifiers before starting.
